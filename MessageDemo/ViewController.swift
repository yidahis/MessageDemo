...
class ViewController: SLKTextViewController {
    ...
    func commonInit() {
        ...
        NotificationCenter.default.addObserver(self,  selector: #selector(ViewController.textInputbarDidMove(_:)), name: NSNotification.Name.SLKTextInputbarDidMove, object: nil)
    }
...
//        self.leftButton.tintColor = UIColor.gray
        ...
        self.tableView.register(YMTableViewCell.classForCoder(), forCellReuseIdentifier: MessengerCellIdentifier)
        
        
        self.autoCompletionView.register(YMTableViewCell.classForCoder(), forCellReuseIdentifier: AutoCompletionCellIdentifier)
...
extension ViewController {
    ...
    func configureActionItems() {
        ...
        let arrowItem = UIBarButtonItem(image: UIImage(named: "icn_arrow_down"), style: .plain, target: self, action: #selector(ViewController.hideOrShowTextInputbar(_:)))
        let editItem = UIBarButtonItem(image: UIImage(named: "icn_editing"), style: .plain, target: self, action: #selector(ViewController.editRandomMessage(_:)))
        let typeItem = UIBarButtonItem(image: UIImage(named: "icn_typing"), style: .plain, target: self, action: #selector(ViewController.simulateUserTyping(_:)))
        let appendItem = UIBarButtonItem(image: UIImage(named: "icn_append"), style: .plain, target: self, action: #selector(ViewController.fillWithText(_:)))
        let pipItem = UIBarButtonItem(image: UIImage(named: "icn_pic"), style: .plain, target: self, action: #selector(ViewController.togglePIPWindow(_:)))
    ...
    func fillWithText(_ sender: AnyObject) {
        ...
        if self.textView.text.utf8CString.count == 0 {
    ...
    func editCellMessage(_ gesture: UIGestureRecognizer) {
        ...
        guard let cell = gesture.view as? YMTableViewCell else {
            return
        }
...
extension ViewController {
    ...
    override func didPressLeftButton(_ sender: Any!) {
        ...
        print("boottom left button pressed")
    }
    ...
    override func didPressArrowKey(_ keyCommand: UIKeyCommand?) {
        ...
        if keyCommand.input == UIKeyInputUpArrow && self.textView.text.utf8CString.count == 0 {
    ...
    override func didChangeAutoCompletionPrefix(_ prefix: String, andWord word: String) {
        ...
        else if (prefix == ":" || prefix == "+:") && word.utf8CString.count > 0 {
...
extension ViewController {
    ...
    func messageCellForRowAtIndexPath(_ indexPath: IndexPath) -> YMTableViewCell {
        
        let cell = self.tableView.dequeueReusableCell(withIdentifier: MessengerCellIdentifier) as! YMTableViewCell
        
        if cell.gestureRecognizers?.count == nil {
            let longPress = UILongPressGestureRecognizer(target: self, action: #selector(ViewController.didLongPressCell(_:)))
        ...
        cell.thumbnailView.image = UIImage.init(named: "avatqr")
        cell.indexPath = indexPath
        cell.usedForMessage = true
        cell.isRight = message.isright
    ...
    func autoCompletionCellForRowAtIndexPath(_ indexPath: IndexPath) -> YMTableViewCell {
        
        let cell = self.autoCompletionView.dequeueReusableCell(withIdentifier: AutoCompletionCellIdentifier) as! YMTableViewCell
        cell.indexPath = indexPath
    ...
    override func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        if tableView == self.tableView {
            ...
            let pointSize = YMTableViewCell.defaultFontSize()
            ...
            if message.text.utf8CString.count == 0 {
                return 0
            }
